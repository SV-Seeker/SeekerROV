...
void  FreeIMU::AHRSupdate(float gx, float gy, float gz, float ax, float ay, float az) {
  ...
  if(halfex != 0.0f && halfey != 0.0f && halfez != 0.0f) {
	...
	if ((micros())<1000000) twoKpFast = twoKp*6;  //if we're in the first 1 second of running, rely very heavily on the mag/acc sensors
	else twoKpFast = twoKp;                       //after that, pay attention less to the mag/acc sensors and more to the gyros
    if(twoKi > 0.0f) {
...
void FreeIMU::getQw(float * q, float * w) {
  float val[9];
  getValues(val);
...
void FreeIMU::getYawPitchRollSpeed(float * yprw) {
  float q[4]; // quaternion
  float w[3]; // angular velocity, body coordinates
  float gx, gy, gz; // estimated gravity direction
  getQw(q, w); 
  ...
  yprw[0] = atan2(2*q[1]*q[2] - 2*q[0]*q[3], 2*q[0]*q[0] + 2*q[1]*q[1] - 1);
  yprw[1] = atan(gx*invSqrt(gy*gy + gz*gz));
  yprw[2] = atan(gy*invSqrt(gx*gx + gz*gz));
  
  gx = sin(yprw[1]);           //reusing these variables for a different purpose
  gy = cos(yprw[1]);           //to avoid computing sin and cos twice
  
  yprw[3] = w[2]*gy+w[0]*gx;  //gyro values are already in degrees per second
  yprw[4] = w[1];             //so just transform between body coordinates
  yprw[5] = w[0]*gy-w[2]*gx;  //and world coordinates
  
  yprw[0] *= 180.0/M_PI;      //transform roll pitch yaw angles to degrees
  yprw[1] *= 180.0/M_PI;
  yprw[2] *= 180.0/M_PI;
}
